 ...
 *
 * <p>Config is optional — if no file is found the loader returns the default
 * configuration so habit-hooks can run without any setup.
 */
public final class ConfigLoader {
    ...
    /**
     * Loads config from the given path, falling back to the default config
    ...
    private static Optional<HabitHooksConfig> readConfig(Path path) {
        ...
        try (InputStream in = Files.newInputStream(path)) {
            ...
            LOGGER.warn("Failed to parse config file {}: {}", path, e.getMessage());
            return Optional.empty();
        }
    }
}
