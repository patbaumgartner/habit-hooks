...
- New coached rules: `pmd:UselessParentheses`, `pmd:UnnecessaryReturn`, `pmd:UnnecessaryModifier`, `checkstyle:UnnecessarySemicolonInEnumeration`, and `checkstyle:UnnecessarySemicolonAfterTypeMemberDeclaration`
- `pmd:GuardLogStatement` coaching ships as available guidance with a commented opt-in block in the bundled ruleset
- Coached-rule count badge backed by `docs/coached-rules-badge.json`, regenerated by `scripts/coached-rules-count.sh` and drift-checked in CI
